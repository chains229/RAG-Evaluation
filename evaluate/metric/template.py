...
**Input:**""" + input(level, question, response, answer, domain="News") + """---
...
**Note:** Ensure evaluations are objective and based solely on the content provided, focusing on how well the Candidate Response reflects the diversity of viewpoints and the debatable nature of the question."""
            ...
            p = """You will be provided with a case brief, a ground truth court decision created by a human expert, and a candidate court decision generated by a language model. Your task is to evaluate the candidate court decision by comparing it to the ground truth court decision, focusing specifically on quality and similarity.

**Evaluation Criteria:**

1. **Accuracy (0-2 points):** Determine whether the candidate court decision is factually correct based on the ground truth.

2. **Reasoning Similarity (0-8 points):** Judge the reasoning similarity in content and meaning between the candidate court decision and the ground truth court decision.
...
**Input:**"""+ input(level, question, response, answer, domain="Law") + """
---
...
---
**Note:** Ensure that your evaluations are objective and based solely on the content provided, focusing on the quality and reasoning similarity between the candidate and ground truth court decision."""
            return p
    elif level == "Create":
...
def input(level: str, question: str, response: str, answer: str, domain: str = "News") -> str:
    if level == "Remember" or level == "Analyze":
        return f"""- **Question**: 
{question}
...
- **Ground Truth court decision:** 
{answer}

- **Candidate court decision:**
{response}
"""
    elif level == "Create":
        return f"""
